...
def main():
    ...
    # Reps model only uses rep-based exercises.
    dataframe = dataframe[
        dataframe["measurement_type"] == "reps"
    ].copy()

    if dataframe.empty:
        raise ValueError(
            "No reps-based training samples found."
        )
    ...
    y_train = train_df["target_reps"]

    X_validation = validation_df[FEATURE_COLUMNS]
    y_validation = validation_df["target_reps"]

    if y_train.isna().any():
        raise ValueError(
            "Training data contains missing rep targets."
        )

    if y_validation.isna().any():
        raise ValueError(
            "Validation data contains missing rep targets."
        )
