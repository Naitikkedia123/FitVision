...
class PlankDetector(BaseExercise):
    """
    Tracks cumulative plank holding time.

    A brief invalid/missing frame does not reset the timer.
    Timing pauses only after the plank posture has been
    invalid continuously for the timeout period.
    """

    START_BODY_ANGLE = 150.0
    START_KNEE_ANGLE = 150.0

    RELEASE_BODY_ANGLE = 145.0
    RELEASE_KNEE_ANGLE = 145.0
    ...
    def process(self, landmarks):
        ...
        if not self._holding:
            ...
            valid = (
                body_angle
                >= self.START_BODY_ANGLE
                and
                knee_angle
                >= self.START_KNEE_ANGLE
            )
            ...
            valid = (
                body_angle
                >= self.RELEASE_BODY_ANGLE
                and
                knee_angle
                >= self.RELEASE_KNEE_ANGLE
            )
        ...
        if self.stage == "holding":
            status = "Hold your plank"
        else:
            status = "Get into a straight plank position"
